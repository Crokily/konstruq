...
export function createDataTools(appUserId: string) {
  return {
    ...
    searchDatasets: tool({
      description:
        "Search across ALL datasets for rows containing a value in any column. Useful when user mentions a project name, vendor, cost code, etc. Returns up to 20 matches per dataset.",
      parameters: z.object({
        searchTerm: z.string().describe("Case-insensitive partial match"),
        category: z.enum(["pm", "erp"]).optional(),
      }),
